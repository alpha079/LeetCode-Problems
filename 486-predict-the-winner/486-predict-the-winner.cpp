...
public:
    ...
    int find(int i, int j, bool turn, vector<int> &array){
        if(i>j) return 0;
        ...
        if(turn){
            int pathA = array[i] + find(i+1, j, 0, array);
            int pathB = array[j] + find(i, j-1, 0, array);
            return  max(pathA, pathB);
        }

        //PLAYER B: Minimize the score of player A.
        else{
            int pathA = find(i+1, j, 1, array);
            int pathB = find(i, j-1, 1, array);
